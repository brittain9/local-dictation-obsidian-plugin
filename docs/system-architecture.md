...
flowchart TB
    ...
    subgraph Sidecar ["Native sidecar (Rust)"]
        ...
        subgraph Text ["Text pipeline"]
            direction LR
            FILT["Hallucination<br/>filter"] --> LLM["LLM postprocess<br/>(optional · boundary-collapsing)"] --> PUNCT["Punctuation"] --> RULES["User rules"]
        end
...
- **Per-utterance LLM postprocess is a text-pipeline stage with an explicit boundary-collapsing exception.** It may rewrite one finalized utterance into one synthetic utterance-span segment, so timestamped sessions disable it in v1. Whole-session LLM rewrite stays a separate experimental artifact, not a transcript stage.
...
Each finalized utterance carries `VoiceActivityEvidence` derived from the same buffered frames as the PCM sent to inference: session-relative audio bounds, retained speech bounds (plain `u64`, collapsing to `audio_start_ms` when no frame met the fixed 0.35 threshold), voiced/unvoiced duration, and compact mean/max probability. The aggregate ships on the wire as `stageResults[0].payload.voiceActivity` for the plugin journal; finality is a typed `isFinal` field on each `StageOutcome`, not part of that payload. The per-frame Silero trace also reaches downstream Rust stages as `StageContext.vad_probabilities` — a borrowed slice, never serialized — so processors that need sub-utterance resolution (e.g. per-segment voiced fraction) compute it via `audio_metadata::voiced_fraction`.
