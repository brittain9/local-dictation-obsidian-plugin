...
function validateTranslationEvent(event: Record<string, unknown>): void {
  ...
  const invalid = (): never => { throw new Error(`Invalid ${type} sidecar event.`); };
  ...
  if (type === 'translation_started') {
    ...
    if (!Number.isSafeInteger(event.completed) || !Number.isSafeInteger(event.total) || (event.completed as number) < 0 || (event.total as number) < 0 || (event.completed as number) > (event.total as number)) invalid();
  } else if (type === 'translation_complete') {
    if (!Array.isArray(event.translations) || !event.translations.every((value) => typeof value === 'string')) invalid();
  } else if (type === 'translation_error') {
    if (typeof event.code !== 'string' || event.code.length === 0 || typeof event.message !== 'string') invalid();
  }
}
